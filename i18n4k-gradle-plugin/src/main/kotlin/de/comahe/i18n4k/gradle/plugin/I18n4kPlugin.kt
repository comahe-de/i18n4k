...
//                    else
    ...
    private fun getGeneratedSourcesDirectory(project: Project, config: I18n4kExtension): File {
        return Companion.getGeneratedSourcesDirectory(project.layout.buildDirectory.get(), config)
    }
    ...
    companion object {
        fun getGeneratedSourcesDirectory(buildDirectory: Directory, config: I18n4kExtension): File {
            val dir = File(
                config.sourceCodeOutputDirectory.replace(
                    "{buildDir}",
                    buildDirectory.asFile.absolutePath
                )
            )
            dir.mkdirs()
            return dir
        ...
        fun getGeneratedLanguageFilesDirectory(
            ...
            val dir = File(
                config.languageFilesOutputDirectory.replace(
                    ...
                    "{projectDir}",
                    projectDirectory.asFile.absolutePath
                )
            )
            dir.mkdirs()
            return dir
