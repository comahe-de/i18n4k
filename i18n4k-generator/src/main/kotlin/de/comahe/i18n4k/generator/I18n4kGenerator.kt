...
class I18n4kGenerator(
    ...
    private fun TypeSpec.Builder.addJsExportAnnotationIfNeeded(): TypeSpec.Builder {
        if(!settings.enableJsExportAnnotation)
            return this

        if (settings.generationTarget == GenerationTargetPlatform.JS
            || settings.generationTarget == GenerationTargetPlatform.MULTI_PLATFORM
        ) {
            addAnnotation(
